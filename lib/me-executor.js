        ...
        if (error) {
            ...
            async.waterfall([
                ...
                function (deviceInfo, innerCallback) {
                    ...
                    if ("040B09050113" === deviceType || "040B09050203" === deviceType) {
                        if ("0001" === message.cmd.cmdCode) {
                            var switchBits = message.cmd.parameters.cmdData & 255;
                            var chanelBits = message.cmd.parameters.cmdData >> 8;
                            logger.debug("message.cmd.parameters.cmdData[before]=" + message.cmd.parameters.cmdData);
                            if (0 === switchBits) {
                                message.cmd.parameters.cmdData = deviceInfo.extra.items.status & (~chanelBits);
                            }
                            else{
                                message.cmd.parameters.cmdData = deviceInfo.extra.items.status|(switchBits&chanelBits)
                            }

                            logger.debug("message.cmd.parameters.cmdData[after]=" + message.cmd.parameters.cmdData);
                        }
                    }
                    //补全温控器设置温度的当前模式，以当前模式为准(注意结构顺序)
                    if ("050608070001" === deviceType
                        && message.cmd.cmdCode === "0003"
                    ) {
                        if(util.isNullOrUndefined(message.cmd.parameters["heat_mode"])){
                            message.cmd.parameters = {
                                "heat_mode":deviceInfo.extra.items.heat_mode,
                                "temp_heat":message.cmd.parameters.temp_heat
                            }
                        }
                        else{
                            message.cmd.parameters = {
                                "heat_mode":deviceInfo.extra.items.heat_mode,
                                "temp_heat":deviceInfo.extra.items.temp_heat
                            }
                        }
                    }
